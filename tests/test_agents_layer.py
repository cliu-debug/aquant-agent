...
from astock_agents.models.analysis_report import (
    ...
    NewsAnalysis, DebateResult, TradeProposal,
...
def create_mock_technical_analysis() -> TechnicalAnalysis:
    """创建模拟技术分析结果"""
    return TechnicalAnalysis(
        ...
        patterns=["金叉突破"],
        summary="技术面偏多，处于上升趋势",
        signal=Signal.BUY,
        confidence=60
    )


def create_mock_fundamental_analysis() -> FundamentalAnalysis:
    """创建模拟基本面分析结果"""
...
def create_mock_sentiment_analysis() -> SentimentAnalysis:
    """创建模拟情绪分析结果"""
    return SentimentAnalysis(
        overall_score=70,
        market_sentiment="市场情绪偏多",
        related_hot_topics=["白酒板块", "消费升级"],
        topic_momentum={"白酒板块": 0.8, "消费升级": 0.6},
        fund_flow="主力资金小幅流入",
        news_sentiment="新闻面偏正面",
        social_sentiment="社交媒体情绪积极",
        summary="市场情绪偏多，资金面向好",
        signal=Signal.BUY,
        confidence=55
    )


def create_mock_news_analysis() -> NewsAnalysis:
    """创建模拟新闻分析结果"""
    return NewsAnalysis(
        ...
        risk_events=[],
        summary="新闻面偏正面，无重大风险",
        signal=Signal.HOLD,
        confidence=50
    )


def create_mock_debate_result() -> DebateResult:
    """创建模拟辩论结果"""
    return DebateResult(
...
def create_mock_trade_proposal() -> TradeProposal:
    """创建模拟交易提案"""
...
class TestRiskManager:
    ...
    def test_create_manager(self):
        ...
        assert manager.name == "风险管理经理"

    def test_assess_risk(self):
        """测试风险评估"""
        manager = RiskManager()
        stock_data = create_mock_stock_data()
        trade_proposal = create_mock_trade_proposal()

        result = manager.analyze(stock_data=stock_data, trade_proposal=trade_proposal)
...
class TestRiskLevelEnum:
    ...
    def test_risk_level_values(self):
        ...
        assert RiskLevel.MODERATE.value == "中等风险"
...
class TestAnalysisReport:
    ...
    def test_report_with_analyses(self):
        """测试带分析结果的报告"""
        tech = create_mock_technical_analysis()
        report = AnalysisReport(
            stock_code="600519.SH",
            stock_name="贵州茅台",
            technical=tech
        ...
        assert report.technical.trend == "上升趋势"

    def test_report_with_full_analyses(self):
        """测试带完整分析结果的报告"""
        report = AnalysisReport(
            stock_code="600519.SH",
            stock_name="贵州茅台",
            technical=create_mock_technical_analysis(),
            fundamental=create_mock_fundamental_analysis(),
            sentiment=create_mock_sentiment_analysis(),
            news=create_mock_news_analysis(),
            debate=create_mock_debate_result(),
            trade_proposal=create_mock_trade_proposal(),
            risk_assessment=RiskAssessment(
                ...
                volatility_risk=50,
                fundamental_risk=45,
                risk_analysis="综合风险中等",
                risk_control_suggestions=["控制仓位"],
                approved=True,
                approval_notes="风险可控"
            )
        )
        ...
        assert report.trade_proposal is not None
        assert report.risk_assessment is not None
        assert report.risk_assessment.risk_level == RiskLevel.MODERATE
...
class TestPatternRecognition:
    ...
    def test_hammer_pattern(self):
        ...
        # 创建锤子线形态
        # open=100, close=100.5, body=0.5
        # lower_shadow=100-90=10, upper_shadow=100.5-100.5=0
        # 满足: lower_shadow(10) > 2*body(1), upper_shadow(0) < body(0.5)
        df = pd.DataFrame({
            'open': [100],
            'high': [100.5],
            'low': [90],   # 长下影线
